...
export async function generateMetadata ({params}) {
    ...
    const course = db.prepare("SELECT * FROM courses WHERE course_slug = ?").get(slug)
    ...
    return (
        <main className="max-w-6xl mx-auto px-4 py-10 grid grid-cols-1 md:grid-cols-[1fr_2fr] gap-8">
            <aside className="md:sticky md:top-6 self-start">
                <CourseCard course={course} showDescription={false} showViewDetailsButton={false}/>
            </aside>
            <section className="flex flex-col">
                <div className="flex flex-col gap-8">
                    ...
                    {thingsToLearn.length > 0 && (
                        <div>
                            <h1 className="text-2xl font-semibold mb-3">What you will learn</h1>

                            <ul className="list-disc last-inside space-y-1 text-gray-700 dark:text-gray-300">
